...
- The class now uses the modern Python 3 `abc.ABC` base class instead of the older `__metaclass__ = ABCMeta` pattern.
- The class does not currently define any `@abstractmethod`s or shared attributes.
...
The implementation annotates `currency` as `str`.
...
`Universe` itself now uses the standard Python 3 `ABC` base class, and `get_assets` is marked with `@abstractmethod`.
...
Because `get_assets` is abstract, concrete subclasses such as `StaticUniverse` and `DynamicUniverse` must provide the implementation.
