...
### Internal frame accessor API

Two new public getters expose the in-memory DataFrames directly, primarily for inspection and debugging.

#### `get_asset_bar_frames() -> dict[str, pd.DataFrame]`

Returns the `self.asset_bar_frames` dictionary — the raw daily OHLCV DataFrames keyed by asset symbol, as loaded from CSV.

#### `get_asset_bid_ask_frames() -> dict[str, pd.DataFrame]`

Returns the `self.asset_bid_ask_frames` dictionary — the converted intraday bid/ask DataFrames keyed by asset symbol.

These accessors are also used by the module's `__main__` entry point (see below).

---

### Running `daily_bar_csv.py` as a script

`daily_bar_csv.py` can be executed directly as a script for quick sanity-checking of CSV loading and price conversion:

```bash
python -m qstrader.data.daily_bar_csv
```

The `__main__` block loads `SPY` with `adjust_prices=True` and `AAPL` with `adjust_prices=False` (using the paths from `qstrader.constants.DATA_DIR`) and prints the first few rows of both the bar frames and the bid/ask frames for each asset.
...
```python
BacktestDataHandler(universe: Universe, data_sources: list[CSVDailyBarDataSource])
```
...
- `data_sources: list[CSVDailyBarDataSource]`
  - A list of data sources to query in order. Now required (no longer optional).
...
- **raises `ValueError`** if `asset_symbol` is not in the universe at `dt`,
...
Calls `get_asset_latest_bid_price` and `get_asset_latest_ask_price` independently and returns `(bid, ask)`.

Because the underlying `CSVDailyBarDataSource` still sets `Bid = Ask = Price`, the pair is numerically identical for daily-bar data. The separation is meaningful once a data source with real spreads is introduced.
...
#### `get_assets_historical_range_close_price(start_dt: pd.Timestamp, end_dt: pd.Timestamp, asset_symbols: list[str]) -> pd.DataFrame`
...
The `adjusted` keyword argument previously forwarded to the data source has been removed; the method now forwards only the three positional arguments.
...
### 5. Universe membership enforced at query time

`BacktestDataHandler` now raises a `ValueError` when `get_asset_latest_bid_price` or `get_asset_latest_ask_price` is called with an asset symbol that is not in the universe at the requested timestamp. Previously this check was absent (marked as a TODO). Code that queries prices for assets outside their active universe window will now fail loudly rather than silently.
...
|---|---|---|---|
| `CSVDailyBarDataSource` | `qstrader.data.daily_bar_csv` | Load and convert daily CSV bars into timestamped price series | `get_bid`, `get_ask`, `get_assets_historical_closes`, `get_asset_bar_frames`, `get_asset_bid_ask_frames` |
