...
- `TopNMomentumAlphaModel` ranks universe assets by lookback-period momentum returns and assigns equal target signal weights to the top N assets.
...
**Source:** `qstrader/alpha_model/top_n_momentum.py`

Generates signal weights by ranking universe assets based on cumulative holding-period returns over a specified lookback period and allocating equal weight ($1/N$) to the top $N$ assets.
...
- `signals: SignalsCollection` — signals collection providing access to `'momentum'` (`MomentumSignal`).
- `mom_lookback: int` — lookback period in business days (e.g. 126 for ~6 months).
- `mom_top_n: int` — number of highest-momentum assets to include in the portfolio ($N$).
- `universe: Universe` — asset universe to evaluate.
- `data_handler: BacktestDataHandler` — data handler instance.
...
def __call__(self, dt: pd.Timestamp) -> dict[str, float]:
    ...
    # Only generate weights if signals warmup window is satisfied
    if self.signals.warmup >= self.mom_lookback:
        weights = self._generate_signals(dt, weights)
    return weights
```

- **Warmup check**: Returns zero weights (`0.0`) for all assets until `signals.warmup >= mom_lookback`.
- **Ranking (`_highest_momentum_asset`)**: Queries `self.signals['momentum'](asset, self.mom_lookback)` for every asset and sorts in descending order.
- **Weight allocation (`_generate_signals`)**: Assigns equal weight `1.0 / mom_top_n` to each of the top $N$ assets, leaving remaining assets at `0.0`.

#### Typical use

Use this model in Tactical Asset Allocation (TAA) and sector rotation strategies (e.g. `examples/momentum_taa.py`).
...
| `TopNMomentumAlphaModel` | `qstrader.alpha_model.top_n_momentum` | Rank assets by momentum & assign 1/N weight to top N | `TopNMomentumAlphaModel(signals, mom_lookback, mom_top_n, universe, data_handler)` |
...
signals = alpha(dt)
# Allocates 0.333 weight to top 3 momentum assets, 0.0 to others
```
