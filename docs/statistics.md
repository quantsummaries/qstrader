...
- Weekly aggregation uses ISO year/week buckets so a calendar year boundary does not split a single ISO week.
- Monthly aggregation returns a MultiIndex keyed by `(year, month)`; yearly aggregation returns a single-year index.

#### Calculation

Compound returns within each grouping period are calculated geometrically as:

$$\text{Compound Return} = \prod_i (1 + r_i) - 1$$
...
Calculates the Compound Annual Growth Rate (CAGR) based on a cumulative equity curve.

- `equity: pd.Series` — time series of portfolio equity or cumulative returns. The first value must be positive.
...
$$\text{CAGR} = \left(\frac{\text{equity}_{\text{final}}}{\text{equity}_{\text{initial}}}\right)^{\frac{1}{\text{Years}}} - 1.0$$
...
- `max_drawdown_pct`: alias of `max_drawdown` used by some report consumers
- `max_drawdown_duration`: duration of maximum drawdown in periods
- `equity`: original `Equity` series from the input DataFrame

##### `plot_results(filename: str|None=None) -> None`

Renders a 7-panel GridSpec visual report (5 rows x 3 columns) containing:
1. **Cumulative Returns Plot** (rows 0-1, full width): strategy vs. optional benchmark equity curves over time.
2. **Drawdown Area Chart** (row 2, full width): underwater drawdown percentage curve over time.
3. **Monthly Returns Heatmap** (row 3, columns 0-1): Seaborn annotated heatmap of monthly percentage returns (Jan-Dec).
4. **Yearly Returns Bar Chart** (row 3, column 2): bar chart of annual percentage returns.
5. **Curve Summary Text Panel** (row 4, column 0): side-by-side strategy and benchmark metrics (Total Return, CAGR, Sharpe Ratio, Sortino Ratio, Annual Volatility, Max Daily Drawdown, Trades per Year).
6. **Trade Text Panel** (row 4, column 1): trade-level metrics (Trade Winning %, Average Trade %, Average Win %, Average Loss %, Best Trade %, Worst Trade %, Worst Trade Date, Avg Days in Trade, Trades).
7. **Time Text Panel** (row 4, column 2): monthly/yearly hit-rate and extreme-return metrics (Winning Months %, Average Winning/Losing Month %, Best/Worst Month %, Winning Years %, Best/Worst Year %).

If `filename` is provided as a string, the figure is saved to that file before display.

##### `_plot_txt_trade(stats: dict, ax: matplotlib.Axes|None=None, **kwargs) -> matplotlib.Axes`

Renders the trade statistics text panel from `stats['returns']` using `_calculate_trade_returns(...)`.

Trade-return definition:
- A **trade return** is the simple sum of a contiguous run of non-zero period returns with the same sign.
- A sign flip (positive to negative or negative to positive) closes the current trade and starts a new one.
- A zero return acts as a delimiter and closes any currently open trade.
- `NaN` returns are ignored before grouping.

Per-field calculations:
- `Trade Winning %` = `len(trade_returns[trade_returns > 0]) / len(trade_returns)`; displayed as `N/A` when no trades exist.
- `Average Trade %` = `mean(trade_returns)`; displayed as `N/A` when no trades exist.
- `Average Win %` = `mean(trade_returns[trade_returns > 0])`; displayed as `N/A` when there are no winning trades.
- `Average Loss %` = `mean(trade_returns[trade_returns < 0])`; displayed as `N/A` when there are no losing trades.
- `Best Trade %` = `max(trade_returns)`; displayed as `N/A` when no trades exist.
- `Worst Trade %` = `min(trade_returns)`; displayed as `N/A` when no trades exist.
- `Worst Trade Date` is currently a placeholder string (`TBD`) and is not computed.
- `Avg Days in Trade` is currently a placeholder string (`0.0`) and is not computed.
...
- `Best Trade %`
- `Worst Trade %`
- `Worst Trade Date` (currently placeholder value `TBD`)
- `Avg Days in Trade` (currently placeholder value `0.0`)
- `Trades` (count of synthetic trades)

##### `_plot_txt_time(stats: dict, ax: matplotlib.Axes|None=None, **kwargs) -> matplotlib.Axes`

Renders time-based summary statistics from monthly/yearly aggregated returns:

Aggregation basis:
- Monthly and yearly returns are generated via `aggregate_returns`, which compounds period returns as:
  - `prod(1 + r_i) - 1`

Per-field calculations:
- `Winning Months %` = `count(monthly_ret >= 0) / count(monthly_ret)` (zero-return months count as winning).
- `Average Winning Month %` = `mean(monthly_ret[monthly_ret >= 0])`.
- `Average Losing Month %` = `mean(monthly_ret[monthly_ret < 0])`.
- `Best Month %` = `max(monthly_ret)`.
- `Worst Month %` = `min(monthly_ret)`.
- `Winning Years %` = `count(yearly_ret >= 0) / count(yearly_ret)` (zero-return years count as winning).
- `Best Year %` = `max(yearly_ret)`.
- `Worst Year %` = `min(yearly_ret)`.

Edge-case notes:
- If a filtered subset is empty (for example, no losing months), NumPy mean/max/min behavior can produce `NaN` for the corresponding display field.
...
All datetime-indexed series and DataFrames in the JSON payload are converted to UTC epoch milliseconds before serialization.
