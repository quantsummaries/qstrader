...
`Signal` is the abstract base class for all rolling price-buffer signal implementations.
...
- `universe: Universe` — the universe of assets for which signals are calculated.
- `lookbacks: list[int]` — number of lookback periods (e.g., business days) to buffer prices for.
...
- `buffers: AssetPriceBuffers` — price buffer storage.
...
Appends a new price point into the buffer for the specified asset.

```python
update_assets(dt: pd.Timestamp) -> None
```

Queries `universe.get_assets(dt)` and adds any newly introduced assets to `self.assets`.
...
Abstract method that calculates and returns the scalar signal value for a given asset and lookback period.
...
Utility class that maintains double-ended queues (`deque`) of asset prices for specified lookback lengths.
...
- `assets: list[str]` — list of asset symbol strings.
- `lookbacks: list[int]` — lookback periods to create buffers for.

#### Key Lookup Key Format

Buffer deques are stored in `self.prices: dict[str, deque[float]]` using keys formatted by `_asset_lookback_key(asset, lookback)` as `f"{asset}_{lookback}"` (e.g. `'EQ:SPY_12'`).
...
Explicitly creates price buffers for a new asset. Raises `ValueError` if the asset already exists in `self.assets`.
...
- **Dynamic asset handling:** If `asset` does not yet exist in `self.prices`, its buffer dictionary is created automatically on the fly.
