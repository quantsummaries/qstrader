...
- `__eq__(rhs: object) -> bool` — returns `True` if both `ts` and `event_type` match between two `SimulationEvent` instances.
- `__eq__` returns `NotImplemented` when `rhs` is not a `SimulationEvent`, allowing Python to fall back to reflected comparison.
...
#### Internal business-day storage

- `_generate_business_days() -> list[pd.Timestamp]` materializes the `BDay` date range into a Python list and stores it on `self.business_days`.
